...
document.addEventListener("pageinit", function(e) {
	...
	{		
		...
		case "menucategory-page":
		   enabled_gallery = getStorage("enabled_gallery");
	       ...
	       enabled_booking = getStorage("enabled_booking");
		   if(enabled_booking==1){
	      	 	$("#menu_tab_booking").show();
	       } else {
	      	 	$("#menu_tab_booking").hide();
	       }
		    	...
		    	if(enabled_advance_search=="yes"){
		    		...
		    		var tab_html='';
		    		 tab_html+='<ons-tabbar position="top" class="menu_tabbar" >';
						  tab_html+='<ons-tab page="page_search.html" label="'+ getTrans('Home','address') +'" icon="ion-home" active="true"></ons-tab>';
						  tab_html+='<ons-tab page="page_search_byname.html" label="'+ getTrans('Cuisine','name') +'" icon="ion-android-restaurant"></ons-tab>';
						  tab_html+='<ons-tab page="page_search_bystreet.html" label="'+ getTrans('Caterers','street') +'" icon="ion-android-restaurant"></ons-tab>';
						  tab_html+='<ons-tab page="page_search_bycuisine.html" label="'+ getTrans('Late Night','cuisine') +'" icon="ion-icecream"></ons-tab>';
						  tab_html+='<ons-tab page="page_search_byfood.html" label="'+ getTrans('Hot Picks','food') +'" icon="ion-soup-can-outline"></ons-tab>';
					 tab_html+='</ons-tabbar>';
	...
	success: function (data) {	  
		...
		if (data.code==1){
			switch (action)
			{
				...
				$(".result-msg").text(data.details.total+" "+getTrans("Restaurants found",'restaurant_found') );
			   ...
			   case "applyVoucher":   
			    ...
			    toastMsg( getTrans("Click cart icon on top right of your screen",'item_added_to_cart') );
			    break;
	...
	if (!empty(params)){
		...
		if( saveCartToDb() ){
		   ...
		   sNavigator.popPage({cancelIfRunning: true}); //back button
			...
			sNavigator.popPage({cancelIfRunning: true}); //back button
			toastMsg(  getTrans("Item added to cart, click cart icon when order selection is complete",'item_added_to_cart') );
		}
...
var lazyLoadSearch = {
  createItemContent: function(index, oldContent) {      	
  	...
  	if(!empty(search_total)){
  		$(".result-msg").text(search_total+" "+getTrans("Restaurants found",'restaurant_found') );
  	}  	  	
...
{
	$.validate({ 	
	    form : '#frm-wing-commit',    
	    borderColorOnError:"#FF0000",
	    onError : function() {      
	    },	    
	    onSuccess : function() {     	      
	       var params = $( "#frm-wing-commit").serialize();
	       params+="&client_token="+getStorage("client_token");
	       params+="&merchant_id="+ getStorage("merchant_id");
	       /*pts*/
	       params+="&earned_points="+ getStorage('earned_points');	    
...
function initiatorRequest(key)
{
	$.validate({ 	
	    form : '#frm-ipay_initiator',    
	    borderColorOnError:"#FF0000",
	    onError : function() {      
	    },	    
	    onSuccess : function() {     	      
	       var params = $( "#frm-ipay_initiator").serialize();
	       params+="&client_token="+getStorage("client_token");
	       params+="&merchant_id="+ getStorage("merchant_id");
	       params+="&key="+key;
	       /*pts*/
	       ...
	       callAjax("initiatorRequest",params);	       
	       return false;
	    }  
	});
}

function ipayMobileMoney(key)
{
	params ='';
	params+="ipay_sid="+ $(".ipay_sid").val();
	params+="&ipay_hash="+ $(".ipay_hash").val();
	params+="&ipay_oid="+ $(".ipay_oid").val();
	params+="&ipay_amount="+ $(".ipay_amount").val();
    params+="&client_token="+getStorage("client_token");
    callAjax("ipayMobileMoney",params);	       
}

function mpesaSendCode()
{
	params="ipay_phone_number="+ urlencode($(".ipay_phone_number").val());
	params+="&ipay_sid="+$(".ipay_sid").val();
	params+="&ipay_hash="+$(".ipay_hash").val();
	params+="&ipay_oid="+$(".ipay_oid").val();
	params+="&client_token="+getStorage("client_token");
	callAjax("ipaympesaSendCode",params);	       
}

function ipayTransactCC()
{
	$.validate({ 	
	    form : '#frm-ipay_card',    
	    borderColorOnError:"#FF0000",
	    ...
	    onSuccess : function() {     	      
	       var params = $( "#frm-ipay_card").serialize();
	       params+="&client_token="+getStorage("client_token");
	       params+="&merchant_id="+ getStorage("merchant_id");
	       params+="&ipay_sid="+ $(".ipay_sid").val();
	       params+="&ipay_hash="+ $(".ipay_hash").val();
	       ...
	       callAjax("ipayTransactCC",params);	       
	       return false;
	    }  
	});
}
...
/*STRIPE IDEAL*/
function payStripeIdeal()
{
	$.validate({ 	
	    form : '#frm-stripe_ideal',    
	    borderColorOnError:"#FF0000",
	    onError : function() {      
	    },	    
	    onSuccess : function() {     	      
	      ...
	      try {	
		      ...
		      var publishable_key = $(".stripe_pub_key").val();
		      var stripe_amount = $(".stripe_amount").val();
		      var stripe_currency = $(".stripe_currency").val();
		      var stripe_descriptor = $(".stripe_descriptor").val();
		      var stripe_client_name = $(".stripe_client_name").val();
		      var stripe_return_url = $(".stripe_return_url").val();	      
		      ...
		      stripe.createSource({
				  ...
				  redirect: {
				    ...
				    if(!empty(result.error)){			    	
				    	onsenAlert(result.error.message);
				    } else {			    				    	
				    	dump(result.source.redirect.url);			    	
				    	//alert(result.source.redirect.url);
				    	setHome();
				    	if(!isDebug()){			    		
				    		var iab = cordova.InAppBrowser;
				    		iab.open( result.source.redirect.url  , '_system');  
				    	} else {
...
function payMollie()
{
	var params = $( "#frm-ipay_card").serialize();
       ...
       /*pts*/
       params+="&earned_points="+ getStorage('earned_points');       
       params+="&order_id="+ $(".mol_order_id").val();
	
	callAjax("payMollie",params);
	return false;
}
/*END START MOLLIE*/
...
{	
	 $.validate({ 	
	    ...
	    onSuccess : function() { 
	       ...
	       if ( $(".save_card:checked").length>0){
	          params+="&save_card="+ $(".save_card").val(); 
	       }
	       
	       callAjax("payDixiPay",params);
	       return false;
	    }  
	});
}

function payDixiPayCards()
{	
	var params = $( "#frm_dixi_cards").serialize();	    
	params+="&order_id="+$(".order_id").val();
	callAjax("payDixiPayCards",params);
}